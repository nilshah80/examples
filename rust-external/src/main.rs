...
async fn main() {
    ...
    println!();
    println!("  Identity Service — External Client Example (Rust)");
    println!("  Web UI:  http://localhost:{port}");
    println!("  API:     /steps/1..6 → Rust crypto → Sidecar");
    println!("  Auth:    HMAC-SHA256");
    println!();
...
async fn step_2(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let body = serde_json::json!({
        ...
        "single_session": true,
        "custom_claims": { "partner_id": "P-1234", "region": "APAC" }
    })
    .to_string();
...
async fn run_cli(config: Config) {
    ...
    println!();
    println!("{AMBER}{BOLD}  ╔══════════════════════════════════════════════════╗{RESET}");
    println!("{AMBER}{BOLD}  ║  Identity Service — External Client (Rust)       ║{RESET}");
    println!("{AMBER}{BOLD}  ║  Auth: HMAC-SHA256 + AES-256-GCM                 ║{RESET}");
    println!(
        "{AMBER}{BOLD}  ║  Client: {:<40} ║{RESET}",
        client_id
    );
    println!("{AMBER}{BOLD}  ╚══════════════════════════════════════════════════╝{RESET}");
    println!();
    ...
    println!("{CYAN}{BOLD}  ── Step 2: Token Issue (HMAC-SHA256 + GCM) ──{RESET}");
