...
// ═══════════════════════════════════════════════════════════════════════════
// API CALL RESULT — returned to JS as JSON string
// Tokens are masked as [PROTECTED_IN_WASM] in all displayed fields
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// HTTP RESPONSE — internal struct for fetch results with headers
// ═══════════════════════════════════════════════════════════════════════════
...
impl SessionContext {
    ...
    /// Clear session and tokens from storage
    #[wasm_bindgen]
    ...
    // These handle the complete encrypt → HTTP → decrypt → store lifecycle
    // Tokens never leave WASM memory; JS only sees masked values
    // ═══════════════════════════════════════════════════════════════════════

    /// Step 2: Issue tokens via sidecar (Basic Auth + AES-256-GCM)
    /// Tokens are stored encrypted in sessionStorage — never returned to JS
    #[wasm_bindgen]
    pub async fn issue_token(&self, request_body_json: &str) -> Result<String, JsValue> {
        ...
        // Build request headers — Authorization from WASM constants (never exposed to JS)
        ...
        request_headers.insert("Authorization".to_string(),
            format!("Basic {}", B64.encode(format!("{}:{}", CLIENT_ID, CLIENT_SECRET))));
        ...
        // Extract tokens and store encrypted — tokens never leave WASM
        ...
        log("Tokens issued and stored encrypted in sessionStorage");

        // Mask sensitive values for UI display
        let mut display_headers = request_headers;
        display_headers.insert("Authorization".to_string(), "Basic [PROTECTED_IN_WASM]".to_string());
    ...
    /// Access token loaded from encrypted storage — never touches JS
    #[wasm_bindgen]
    ...
    /// Loads access_token from encrypted storage, creates new session,
    /// migrates encrypted tokens to new session's derived key
    #[wasm_bindgen]
    pub async fn refresh_session(&self) -> Result<SessionContext, JsValue> {
        // Load tokens from encrypted storage using current session's key
        let access_token = load_token_from_storage(
            "encrypted_access_token", &self.session_id, &self.client_id)?;
        let refresh_token = load_token_from_storage(
            "encrypted_refresh_token", &self.session_id, &self.client_id)?;

        // Perform authenticated ECDH to get new session
        ...
        // Re-encrypt tokens with new session's derived key
    ...
    /// Both tokens loaded from encrypted storage, updated tokens re-encrypted
    #[wasm_bindgen]
    ...
    /// Tokens loaded from encrypted storage, all storage cleared after revocation
    #[wasm_bindgen]
    pub async fn revoke_tokens(&self) -> Result<String, JsValue> {
        ...
        // Response may have empty payload for revocation
        ...
        // Clear all session and token data from storage
...
/// Internal session init logic — called by both init_session (JS) and refresh_session
...
/// Initialize session with ECDH key exchange (JS entry point)
#[wasm_bindgen]
...
// Tokens are encrypted with the same per-session derived key used for
// session key storage, with AAD to differentiate token types
// ═══════════════════════════════════════════════════════════════════════════
...
/// Mask token values in JSON for safe display to JS
...
/// Parse idempotency key "timestamp.nonce" into components
...
/// Simple HTTP POST using fetch API (body-only response)
...
/// HTTP POST with response headers (for API call methods)
