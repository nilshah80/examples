...
var subject = app.Configuration["Client:Subject"] ?? "test-user";
...
app.MapPost("/steps/2", async (IdentityService identitySvc, JourneyState state) =>
{
    ...
    var body = JsonSerializer.Serialize(new
    {
        ...
        single_session = true,
        custom_claims = new { roles = "admin", tenant = "test-corp" }
    });
...
var port = app.Configuration.GetValue("Server:Port", 3500);
app.Urls.Add($"http://localhost:{port}");

Console.WriteLine();
Console.WriteLine("  Identity Service — Internal Client Example (.NET 8)");
Console.WriteLine($"  Web UI:  http://localhost:{port}");
Console.WriteLine($"  API:     /steps/1..6 → .NET crypto → Sidecar");
Console.WriteLine($"  Auth:    Basic Auth");
Console.WriteLine();
...
static async Task RunCli(IServiceProvider services, IConfiguration config)
{
    ...
    var subject = config["Client:Subject"] ?? "test-user";
    var clientId = config["Client:Id"] ?? "dev-client";

    Console.WriteLine();
    Console.WriteLine($"{BLUE}{BOLD}  ╔══════════════════════════════════════════════════╗{RESET}");
    Console.WriteLine($"{BLUE}{BOLD}  ║  Identity Service — Internal Client (.NET 8)     ║{RESET}");
    Console.WriteLine($"{BLUE}{BOLD}  ║  Auth: Basic Auth + AES-256-GCM                  ║{RESET}");
    Console.WriteLine($"{BLUE}{BOLD}  ║  Client: {clientId,-40} ║{RESET}");
    Console.WriteLine($"{BLUE}{BOLD}  ╚══════════════════════════════════════════════════╝{RESET}");
    Console.WriteLine();
    ...
    // Step 2: Token Issue
    PrintStep(2, "Token Issue (Basic Auth + GCM)");
