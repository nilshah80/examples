...
// IssueToken performs Step 2: Token Issue (HMAC-SHA256 + GCM).
...
func IssueToken(config *Config, httpClient *http.Client, session *SessionContext, plaintext string) StepResult {
	...
	if err != nil {
		return errorResult(2, "Token Issue (HMAC-SHA256 + GCM)", plaintext, start, err.Error())
	}
	...
	// Send HTTP POST
	url := fmt.Sprintf("%s/api/v1/token/issue", config.SidecarURL)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return StepResult{
			...
			Success:               false,
			Error:                 strPtr(fmt.Sprintf("HTTP %d: %s", resp.StatusCode, decrypted)),
		}
	}
...
// postEncrypted: core encrypted POST for Steps 3, 5, 6 (Bearer + GCM).
